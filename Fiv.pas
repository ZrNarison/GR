...
uses
  ...
  System.IOUtils, System.Rtti, FMX.Grid.Style, FMX.ScrollBox, FMX.Grid,
  FireDAC.Comp.Client, FireDAC.Stan.Param, Data.DB, FMX.DateTimeCtrls,
...
uses Reunion, base, Rfiv, System.UIConsts;

procedure TForm3.DisableChamp;
begin
   Edit1.Enabled:=false;
   Edit2.Enabled:=false;
   Edit3.Enabled:=false;
...
procedure TForm3.Initialisation;
begin
...
begin
  ...
  Form1.Top := Form3.Top;
  Form1.Width := Form3.Width;
  Form1.Height := Form3.Height;
  Form1.Show;
end;
...
procedure TForm3.VerficationChamp;
var
  QMat: TFDQuery;
  dateReunion: TDate;
begin
...
procedure TForm3.AjusterColonnes;
var
  i: Integer;
  ColWidth: Single;
begin
...
begin
  Initialisation;
//  ChargerPresence; // Remplit la grille et StatusArray
end;

procedure TForm3.GridPresenceDrawColumnCell(Sender: TObject; const Canvas: TCanvas;
  const Column: TColumn; const Bounds: TRectF; const Row: Integer; const Value: TValue;
  const State: TGridDrawStates);
var
...
//          dateInscription := QCheck.FieldByName('date_inscription').AsDateTime
...
procedure TForm3.ChargerPresence;
var
    ...
    try
      ...
      begin
        ...
        begin
          ...
          QPres.Close;
          QPres.SQL.Text := 'SELECT valeur, remarque FROM t_presence WHERE idMembre = :idm AND idReunion = :idr';
...
begin
  case ComboBox2.ItemIndex of
    ...
    1,2: begin // En retard ou Présent
         Edit7.Text := '';
         Edit7.Enabled := True;
       end;
  end;
...
procedure TForm3.ComboEdit2Change(Sender: TObject);
begin
//  ShowMessage(ComboEdit2.Text);
  VerficationChamp;
  ChargerPresence;
end;

procedure TForm3.ComboEdit2Exit(Sender: TObject);
begin
  VerficationChamp;
end;
...
begin
  ...
  Form6.Left := Form3.Left;
  Form6.Top := Form3.Top;
  Form6.Width := Form3.Width;
  Form6.Height := Form3.Height;
  Form6.Show;
  Form3.Hide;
end;
...
FS := TFormatSettings.Create;
  FS.DateSeparator := '-';
  FS.ShortDateFormat := 'dd/mm/yyyy'; // format standard SQL
...
//    QCheck.ParamByName('d').AsDate := StrToDate(ComboEdit2.Text);
//    QCheck.ParamByName('d').AsString := FormatDateTime('dd/mm/yyyy', StrToDateDef(ComboEdit2.Text, 0, FS));
...
// Try parsing robustement : d'abord selon la locale, sinon remplacer virgule par point
  ...
  try
    QCheck.Connection := DataModule1.FDConnection1;
    QCheck.SQL.Text := 'SELECT id FROM t_presence WHERE idMembre = :m AND idReunion = :r';
    QCheck.ParamByName('m').AsInteger := IdMembre;
    QCheck.ParamByName('r').AsInteger := newReunionId;
    QCheck.Open;
    if not QCheck.Eof then
    begin
      ShowMessage('Présence déjà enregistrée pour ce membre.');
      Exit;
    end;
  finally
    QCheck.Free;
    ...
    // 8)Comparaison de la date de réunion avec la date du jour
      if (ComboEdit2.Text <> DateToStr(Date)) then
      begin
        ShowMessage('Seul le jour de la date de réunion peut présenter le membre. Désolé pour ' + Edit1.Text);
        Exit;
      end
      else
      begin
        // Insertion dans la table t_presence
        QInsert := TFDQuery.Create(nil);
        try
          QInsert.Connection := DataModule1.FDConnection1;
          QInsert.SQL.Text :=
            'INSERT INTO t_presence (idMembre, idReunion, remarque, valeur) ' +
            'VALUES (:m, :r, :rem, :val)';
...
//        ShowMessage('Présence enregistrée avec succès.');
      end;
end;
...
begin
  ...
  try
    ...
    // Trouver idMembre depuis le nom complet (gère le cas où le prénom est vide)
    QCheck.SQL.Text :=
      'SELECT id FROM t_membre WHERE TRIM(nom || " " || IFNULL(prenom, "")) = :np';
    QCheck.ParamByName('np').AsString := nomPrenom;
    QCheck.Open;

    // Si aucun résultat, essayer avec seulement le nom (au cas où le prénom est vide)
    if QCheck.IsEmpty then
    begin
      QCheck.Close;
      QCheck.SQL.Text := 'SELECT id FROM t_membre WHERE nom = :n';
      QCheck.ParamByName('n').AsString := nomPrenom;
      QCheck.Open;
    end;
    ...
    // Supprimer dans t_presence
    QDel.SQL.Text := 'DELETE FROM t_presence WHERE idMembre = :idm AND idReunion = :idr';
    QDel.ParamByName('idm').AsInteger := idMembre;
    QDel.ParamByName('idr').AsInteger := idReunion;
    QDel.ExecSQL;

    ShowMessage('La présence sélectionnée a été supprimée avec succès.');
