...
begin
  ...
  try
    ...
    begin
      if (Button2.Text = '➕ Ajouter') or
         ((Button2.Text = '✏️ Modifier') and (QCheck.FieldByName('matricule').AsInteger <> NextMatricule)) then
      begin
...
begin
  ...
  // --- 1) Vérifier si le membre a déjà participé à une social
  with DataModule1.FDQuery1 do
  begin
    Close;
    SQL.Text :=
  ...
  begin
    TDialogService.ShowMessage(
      'Impossible de supprimer ce membre : il a déjà participé ajouté une social.'
    );
    Exit;
    Vidage;
  end;
